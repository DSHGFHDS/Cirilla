...
public class RefTypes : MonoBehaviour
{
    ...
    private IObserverModule observerModule;
    private DataPanel dataPanel;

    class Tevent : UnityEvent<Vector3> { }
    Tevent tevent;

    private void RefUnityEngine()
    {
        Debug.Log(GetTypes());
        ...
        Enum.TryParse<IntEnum>("", out var test);
        tevent.AddListener(pos => { });
        gameObject.GetComponent<Canvas>().renderMode = RenderMode.WorldSpace;
        gameObject.GetComponent<Canvas>().worldCamera = Camera.current;
    }

    private void RefCirilla()
    {
        observerModule.Add(IntEnum.A, objects => { });
        observerModule.Remove(IntEnum.A, objects => { });
        observerModule.Dispatch(IntEnum.A);
        dataPanel.TryGetValue("", out int d1, 0);
        dataPanel.TryGetValue("", out float d2, 0);
        dataPanel.TryGetValue("", out long d4, 0);
        dataPanel.TryGetValue("", out bool d5, 0);
        dataPanel.TryGetValue("", out double d6, 0);
        dataPanel.TryGetValue("", out string d7, 0);
        dataPanel.TryGetValue("", out Object d8, 0);
        dataPanel.TryGetValue("", out Color d9, 0);
        dataPanel.TryGetValue("", out Vector2 d10, 0);
        dataPanel.TryGetValue("", out Vector3 d11, 0);
    }

    private void RefNullable()
    {
        object b = null;
        int? a = 5;
        b = a;
    ...
    private void RefContainer()
    {
        List<object> b = new List<object>()
        {
            ...
            // new Dictionary<int, Dictionary<IntEnum, List<object>>>(),
            // new Dictionary<IntEnum, Dictionary<int, List<object>>>(),
